...
public class Engine {
    ...
    protected static final int STEP_LIMIT = World.STEPLIMIT;
    protected static final int SCORE_LIMIT = World.SCORELIMIT;
        ...
        while (inputSource.possibleNextInput()) {
            ...
            if (!worldInitialized) {
                if (nextKey == 'N') {
                    ...
                    this.SEED = Long.parseLong(seed);
                    this.mapGen = new World(this.SEED);
    ...
    private void replaySavedGame() {
        try {
            ...
            if (loadingSource.getNextKey() == 'R') {
                ...
                replayMode = true;
                mapGen = new World(Long.parseLong(scannedSeed));
    ...
    /** Moves the avatar pieces within the given inputWorld using the given input World
     * object and will render depending on the boolean passed into the method. */
    private void moveAvatar(TETile[][] inWorld, World inMapGen, char key, boolean render) {
        if (key == 'W') {
        ...
        if (inMapGen.score == World.SCORELIMIT) {
